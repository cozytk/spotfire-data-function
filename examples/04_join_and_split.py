'''---
title: 두 테이블 조인 후 조건별로 여러 테이블로 분리 출력
level: 1
difficulty: ★★☆
summary: merge + indicator 로 조인 실패까지 진단하고, 결과를 공정별 3개 테이블로 쪼개 한 번에 내보냅니다. 기존 실습 스크립트의 개선판.
data: [fab_measurement.csv, lot_master.csv]
inputs:
  input1:
    file: fab_measurement.csv
  input2:
    file: lot_master.csv
outputs: [output1, output2, output3, output_unmatched]
---
## 시나리오
