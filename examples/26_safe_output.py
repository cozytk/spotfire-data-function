'''---
title: 출력 안전장치 — Spotfire가 결과를 거부하지 않게 만들기
level: 4
difficulty: ★★☆
summary: 계산은 다 끝났는데 마지막 내보내기에서 실패하는 상황을 없앱니다. 인덱스 소실·전부 결측 컬럼·셀 안의 list·혼합 타입을 한 함수로 정리하는 재사용 가능한 안전장치를 만듭니다.
data: [fab_measurement.csv]
inputs:
  input:
    file: fab_measurement.csv
  min_count: 30
outputs: [output, output_issue]
---
## 시나리오
...
- `spotfire.set_spotfire_types()` 는 **틀린 타입 이름을 줘도 경고만 내고 넘어갑니다.**
  그래서 이 코드는 [문서에 있는 12개 타입 이름](../../syntax/07-spotfire-module/#지정할-수-있는-타입-이름) 중에서만 고릅니다.
