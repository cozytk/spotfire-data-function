'''---
title: 시간 근접 조인 (merge_asof) — 계측 시점의 설비 상태 붙이기
level: 3
difficulty: ★★★
summary: 키가 정확히 일치하지 않는 두 시계열을 "가장 가까운 직전 값" 기준으로 조인합니다. Spotfire 기본 기능으로는 불가능에 가까운 작업.
data: [fab_measurement.csv, equipment_sensor_wide.csv]
inputs:
  input1:
    file: fab_measurement.csv
  input2:
    file: equipment_sensor_wide.csv
  tolerance_min: 60
outputs: [output, output_stats]
---
## 시나리오
