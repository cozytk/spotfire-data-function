'''---
title: SPC 관리도 + 공정능력지수(Cp/Cpk) + Western Electric 룰
level: 3
difficulty: ★★★
summary: 관리한계선을 직접 계산하고, 8개 룰 중 4개를 구현해 "관리 이탈" 시점을 자동으로 찾아냅니다. 데이터 함수의 진가가 가장 잘 드러나는 예제.
data: [fab_measurement.csv]
inputs:
  input:
    file: fab_measurement.csv
  sigma_k: 3.0
outputs: [output, output_capability, output_violation]
---
## 시나리오
