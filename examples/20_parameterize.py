'''---
title: 문서 속성으로 파라미터화 — 한 번 만들어 계속 쓰는 데이터 함수
level: 4
difficulty: ★★★
summary: 그룹 컬럼·집계 방식·기간·임계값을 전부 Value 입력으로 받아, 코드를 고치지 않고 화면에서 조작하는 범용 요약 함수를 만듭니다.
data: [fab_measurement.csv]
inputs:
  input:
    file: fab_measurement.csv
  group_cols: EQP_ID;STEP_DESC
  value_col: THICKNESS
  agg_list: mean;std;median;count
  date_from: "2026-03-03"
  date_to: "2026-03-08"
  threshold: 3.0
outputs: [output, output_meta]
---
## 시나리오
