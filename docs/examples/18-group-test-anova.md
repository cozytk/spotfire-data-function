...
# 18. 설비 간 차이가 진짜인가 — ANOVA와 사후 다중비교
{: .no_toc }

**난이도** ★★★ · **Level 4 · 통계·자동화·Spotfire 고유 기능**

눈으로 보면 달라 보이는 그룹 평균이 통계적으로 유의한지 검정하고, 어느 쌍이 다른지까지 짚어 냅니다.

> **추가 패키지 필요**: `scipy` — `도구 > Python 도구 > 패키지 관리` 에서 설치

사용 데이터: [`fab_measurement.csv`](https://github.com/cozytk/spotfire-data-function/blob/main/data/fab_measurement.csv) · [스크립트 원본](https://github.com/cozytk/spotfire-data-function/blob/main/examples/18_group_test_anova.py)
...
| 입력 | `alpha` | Value | 문서 속성 (예: `0.05`) |
| 출력 | `output_anova` | Table | 새 데이터 테이블 |
| 출력 | `output_posthoc` | Table | 새 데이터 테이블 |
