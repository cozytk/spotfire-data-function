...
# 19. 파레토 분석과 ABC 등급 분류
{: .no_toc }

**난이도** ★★☆ · **Level 4 · 통계·자동화·Spotfire 고유 기능**

누적 기여율을 계산해 "상위 몇 개가 전체의 80%를 차지하는가"에 답하고, A/B/C 등급을 자동으로 매깁니다.

사용 데이터: [`sales_orders.csv`](https://github.com/cozytk/spotfire-data-function/blob/main/data/sales_orders.csv) · [스크립트 원본](https://github.com/cozytk/spotfire-data-function/blob/main/examples/19_pareto_abc.py)
...
| 구분 | 이름 | 타입 | 연결 |
|---|---|---|---|
| 입력 | `input` | Table | `sales_orders.csv` |
| 입력 | `a_cut` | Value | 문서 속성 (예: `0.8`) |
| 입력 | `b_cut` | Value | 문서 속성 (예: `0.95`) |
