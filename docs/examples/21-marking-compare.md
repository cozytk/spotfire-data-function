...
# 21. 마킹(Marking) 입력 활용 — 선택한 구간 vs 나머지 비교
{: .no_toc }

**난이도** ★★★ · **Level 4 · 통계·자동화·Spotfire 고유 기능**

Spotfire에서만 가능한 입력 방식. 사용자가 차트에서 드래그로 고른 행을 그대로 함수의 입력으로 받아 "이 구간이 뭐가 다른가"를 즉시 계산합니다.

사용 데이터: [`fab_measurement.csv`](https://github.com/cozytk/spotfire-data-function/blob/main/data/fab_measurement.csv) · [스크립트 원본](https://github.com/cozytk/spotfire-data-function/blob/main/examples/21_marking_compare.py)
...
| 구분 | 이름 | 타입 | 연결 |
|---|---|---|---|
| 입력 | `input_marked` | Table | `fab_measurement.csv` (마킹/필터로 제한) |
| 입력 | `input_all` | Table | `fab_measurement.csv` |
| 출력 | `output` | Table | 새 데이터 테이블 |
| 출력 | `output_profile` | Table | 새 데이터 테이블 |
